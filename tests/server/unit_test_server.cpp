...
TEST(Options, logo_path) {
  ...
  auto args = fastocloud::server::options::ValidateConfig(cfg);
  ASSERT_EQ(args.size(), 1);

  cfg = "{\"" LOGO_FIELD "\" : {\"path\": \"http://home/user/logo.png\"}}";
  args = fastocloud::server::options::ValidateConfig(cfg);
  ASSERT_EQ(args.size(), 1);
}

TEST(Options, cfgs) {
  auto args = fastocloud::server::options::ValidateConfig(kTimeshiftRecorderConfig);

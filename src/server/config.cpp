...
namespace {
common::ErrnoError ReadSlaveConfig(const std::string& path, fastocloud::utils::ArgsMap* args) {
  if (!args) {
  ...
  fastocloud::utils::ArgsMap options;
  std::string line;
  while (getline(config, line)) {
    const std::pair<std::string, std::string> pair = fastocloud::utils::GetKeyValue(line, '=');
...
common::ErrnoError load_config_from_file(const std::string& config_absolute_path, Config* config) {
  ...
  if (!utils::ArgsGetValue(slave_config_args, SERVICE_STREAMLINK_PATH, &streamlink_path)) {
    streamlink_path = STREAMER_SERVICE_STREAMLINK_PATH;
  }
  lconfig.streamlink_path = streamlink_path;

...
common::Error make_config(const utils::ArgsMap& config_args, Config** config) {
  ...
  if (stream_type == SCREEN) {
    ...
    *config = rconfig;
    ...
    std::string deep_learning_str;
    if (utils::ArgsGetValue(config_args, DEEP_LEARNING_FIELD, &deep_learning_str)) {
      machine_learning::DeepLearning deep_learning;
      common::Error err = deep_learning.DeSerializeFromString(deep_learning_str);
      if (!err) {
        econfig->SetDeepLearning(deep_learning);
      }
    }

    std::string deep_learning_overlay_str;
    if (utils::ArgsGetValue(config_args, DEEP_LEARNING_OVERLAY_FIELD, &deep_learning_overlay_str)) {
      machine_learning::DeepLearningOverlay deep_learning_overlay;
      common::Error err = deep_learning_overlay.DeSerializeFromString(deep_learning_overlay_str);
      if (!err) {
    ...
    std::string log_str;
    if (utils::ArgsGetValue(config_args, LOGO_FIELD, &log_str)) {
      Logo logo;
      common::Error err = logo.DeSerializeFromString(log_str);
      if (!err) {
